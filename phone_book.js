let arrPeople = [{
    name: 'Глеб Вадимов',
    phone: "23-43-43",
    mobile: 89273543453,
    email: 'corporation@mail.ru',
    position: 'Начальник отдела',
    department: 'Отдел разработки ПО'
},{
    name: 'Алексей Шульгин',
    phone: "65-83-89",
    mobile: 89141657879,
    email: 'corporation@mail.ru',
    position: 'Начальник отдела',
    department: 'Отдел рыбной промышленности'
}, {
    name: 'Иван Марусеев',
    phone: "78-45-23",
    mobile: 8925343543,
    email: 'corporation@mail.ru',
    position: 'Начальник отдела',
    department: 'Отдел взаимодействия со СМИ'
}, {
    name: 'Роман Власов',
    phone: "78-93-90",
    mobile: 89256544564,
    email: 'corporation@mail.ru',
    position: 'Начальник отдела',
    department: 'Отдел правового обеспечения'
}, {
    name: 'Петр Сергеев',
    phone: "78-55-67",
    mobile: 89141657879,
    email: 'corporation@mail.ru',
    position: 'Начальник отдела',
    department: 'IT'
}, {
    name: 'Сергей Васильев',
    phone: "34-55-38",
    mobile: 89256544564,
    email: 'corporation@mail.ru',
    position: 'Заместитель',
    department: 'IT'
}, {
    name: 'Иван Пронин',
    phone: "23-42-34",
    mobile: 8925343543,
    email: 'corporation@mail.ru',
    position: 'Специалист',
    department: 'Кадровое обеспечение'
}, {
    name: 'Василий Алтаев',
    phone: "23-43-43",
    mobile: 89273543453,
    email: 'corporation@mail.ru',
    position: 'Инженер',
    department: 'Финансовая служба'
}, {
    name: 'Федор Константинополев',
    phone: "45-43-39",
    mobile: 89255645656,
    email: 'corporation@mail.ru',
    position: 'Инструктор',
    department: 'Маркетинг'
}, {
    name: 'Арсений Петров',
    phone: "78-55-67",
    mobile: 89141657879,
    email: 'corporation@mail.ru',
    position: 'Инженер',
    department: 'IT'
}, {
    name: 'Юрий Плотников',
    phone: "34-55-38",
    mobile: 89256544564,
    email: 'corporation@mail.ru',
    position: 'Администратор',
    department: 'IT'
},  {
    name: 'Константин Перцев',
    phone: "23-43-43",
    mobile: 89273543453,
    email: 'corporation@mail.ru',
    position: 'Администратор',
    department: 'Маркетинг'
}, {
    name: 'Владимир Песков',
    phone: "45-43-39",
    mobile: 89255645656,
    email: 'corporation@mail.ru',
    position: 'Инструктор',
    department: 'Маркетинг'
}, {
    name: 'Андрей Мирославов',
    phone: "78-98-45",
    mobile: 89255645656,
    email: 'corporation@mail.ru',
    position: 'Специалист',
    department: 'Отдел взаимодействия со СМИ'
}, {
    name: 'Борис Иванов',
    phone: "78-55-67",
    mobile: 89141657879,
    email: 'corporation@mail.ru',
    position: 'Инженер',
    department: 'Отдел разработки ПО'
}, {
    name: 'Станислав Рогов',
    phone: "34-55-38",
    mobile: 89256544564,
    email: 'corporation@mail.ru',
    position: 'Специалист',
    department: 'Отдел правового обеспечения\''
}, {
    name: 'Владлен Коробов',
    phone: "23-42-34",
    mobile: 8925343543,
    email: 'corporation@mail.ru',
    position: 'Специалист',
    department: 'Отдел рыбной промышленности'
},
]
